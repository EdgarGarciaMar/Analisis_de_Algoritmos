...
/*
Búsqueda lineal fue modificada, ahora recive el arreglo A, el inicio del intervalo, el final del inter
valo de trabajo la clave que se busca.
*/
int busquedaLineal(int A[], int inicio, int final, int clave)
{
    int i;
    for (i = inicio; i < final; i++)
    {
        if (A[i] == clave)
        { //se hace una comparación del número a buscar con cada posición de arreglo 
            return i;
        }
    }
    return -1;
