gcc compres.c -o comp
#Ejecutar uno por uno, porque el programa se hace bolas
time(./comp entrada.txt) >> frecuencias.txt
time(./comp entrada2.txt) >> frecuencias.txt
time(./comp entrada3.txt) >> frecuencias.txt
time(./comp entrada4.txt) >> frecuencias.txt
time(./comp entrada5.txt) >> frecuencias.txt
time(./comp entrada6.txt) >> frecuencias.txt
time(./comp entrada7.txt) >> frecuencias.txt
time(./comp entrada8.txt) >> frecuencias.txt
time(./comp entrada9.txt) >> frecuencias.txt
